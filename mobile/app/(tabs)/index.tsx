...
export default function Home() {
  ...
  return <ScreenContainer><FlatList
    ...
    renderItem={({ item }) => {
      ...
      return <Actions onCampaign={() => router.push("/(tabs)/campaigns" as never)} onPlan={() => router.push("/(tabs)/plan" as never)} onBrain={() => router.push("/(tabs)/intelligence" as never)} onAutomate={() => router.push("/(tabs)/automate" as never)} onPlatforms={() => router.push("/platforms" as never)} onKnowledge={() => router.push("/knowledge" as never)} onOptimization={() => router.push("/optimization" as never)} onPersonalBrand={() => router.push("/personal-brand" as never)} onDevice={() => router.push("/mobile-companion" as never)} />;
    }}
  /></ScreenContainer>;
}
