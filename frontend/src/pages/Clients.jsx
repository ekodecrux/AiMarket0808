    ...
    <div className="p-5 lg:p-8">
      ...
      {loading ? <Loader /> : clients.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {clients.map((c) => (
            <Fade key={c.id}>
              <div className={`border bg-white ${activeClientId === c.id ? "border-[#FF3B30]" : "border-border"}`} data-testid={`client-${c.id}`}>
                <div className="p-5 border-b border-border">
                  <div className="flex items-start justify-between">
                    <div className="w-10 h-10 border border-zinc-200 flex items-center justify-center"><Buildings size={20} className="text-[#FF3B30]" /></div>
                    <div className="flex items-center gap-1">
                      <button onClick={() => setEditing(c)} className="text-zinc-500 hover:text-white transition-colors duration-200 p-1"><Pencil size={15} /></button>
                      <button onClick={() => del(c.id)} data-testid={`delete-client-${c.id}`} className="text-zinc-500 hover:text-[#FF3B30] transition-colors duration-200 p-1"><Trash size={15} /></button>
                    </div>
                  </div>
                ...
                <div className="p-4 space-y-2">
                  <button onClick={() => setActive(activeClientId === c.id ? "" : c.id)} data-testid={`select-client-${c.id}`}
                    className={`w-full py-2 text-xs uppercase tracking-wider transition-colors duration-200 ${activeClientId === c.id ? "bg-[#FF3B30] text-white" : "border border-zinc-300 text-zinc-700 hover:border-[#FF3B30] hover:text-[#FF3B30]"}`}>
